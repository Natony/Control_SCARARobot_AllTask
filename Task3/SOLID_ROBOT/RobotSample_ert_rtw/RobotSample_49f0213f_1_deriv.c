...
#include "RobotSample_49f0213f_1_geometries.h"

PmfMessageId RobotSample_49f0213f_1_compDerivs(const RuntimeDerivedValuesBundle *
  ...
  NeuDiagnosticManager *neDiagMgr)
{
  ...
  if (ii[0] != 0) {
    return sm_ssci_recordRunTimeError(
      ...
      "'RobotSample/Subsystem/Cylindrical1' has a degenerate mass distribution on its follower side.",
      neDiagMgr);
  }
  ...
  if (ii[0] != 0) {
    return sm_ssci_recordRunTimeError(
      "sm:compiler:messages:simulationErrors:DegenerateMass",
      "'RobotSample/Subsystem/Cylindrical' has a degenerate mass distribution on its follower side.",
      neDiagMgr);
  }
...
PmfMessageId RobotSample_49f0213f_1_numJacPerturbLoBounds(const
...
PmfMessageId RobotSample_49f0213f_1_numJacPerturbHiBounds(const
  RuntimeDerivedValuesBundle *rtdv, const int *eqnEnableFlags, const double
  *state, const int *modeVector, const double *input, const double *inputDot,
  const double *inputDdot, const double *discreteState, double *bounds, double
  *errorResult, NeuDiagnosticManager *neDiagMgr)
