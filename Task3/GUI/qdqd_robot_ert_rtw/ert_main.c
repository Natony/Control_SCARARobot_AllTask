...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(qdqd_robot_M, 0);
  ...
  qdqd_robot_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(qdqd_robot_M->extModeInfo, &rtmGetTFinal(qdqd_robot_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(qdqd_robot_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(qdqd_robot_M);
