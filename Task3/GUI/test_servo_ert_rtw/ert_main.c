...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(test_servo_M, 0);
  ...
  test_servo_initialize();
  cli();
  sei ();

  /* External Mode initialization */
  errorCode = extmodeInit(test_servo_M->extModeInfo, &rtmGetTFinal(test_servo_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(test_servo_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(test_servo_M);
